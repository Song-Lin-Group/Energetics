...
This script takes a set of Gaussian output files containing and extracts
calculated quantum mechanical descriptors for labeled atoms in each Gaussian
file as defined in 'Atomlabels'. The input files must be in the same working
directory as this script.

The results are tabulated and recorded in an output Excel workbook.
...
def get_coords_com(file):
    dir = "./"
    filecont = open(dir+file+".gjf",'r').readlines()
    coords,elements = [],[]
    start = 999
...
# the get_nbo_orbs() and get_nbo_overlap() functions are project specific and require changing the orbital and atom identification manually in the script
        ...
        if "NATURAL BOND ORBITAL ANALYSIS:" in filecont[i]:
            for j in range(i+10,len(filecont)):
                if " LP ( 1) P" in filecont[j]:
                    s_lp = float_pattern.findall(filecont[j])[1]
                    break
        if "Natural Bond Orbitals (Summary)" in filecont[i]:
            for j in range(i,len(filecont)):
                if " LP" in filecont[j] and "P " in filecont[j]:
                    elp = float_pattern.findall(filecont[j])[-1]
                if "RY*" in filecont[j] and "P " in filecont[j] and float(float_pattern.findall(filecont[j])[-1]) < float(rys):
                    rys = float_pattern.findall(filecont[j])[-1]

                if "BD (   1)" in filecont[j] and "P " in filecont[j]:# and " - " in filecont[j]:
                    bd.append(float(float_pattern.findall(filecont[j])[-1]))
                # elif "BD (   1)" in filecont[j] and " P " in filecont[j]:
                #     bd.append(float(filecont[j].split()[9]))
                if "BD*(   1)" in filecont[j] and "P " in filecont[j]:# and " - " in filecont[j]:
                    bds.append(float(float_pattern.findall(filecont[j])[-1]))
                # elif "BD*(   1)" in filecont[j] and " P " in filecont[j]:
                #     bds.append(float(filecont[j].split()[8]))

                if "BD (   2)" in filecont[j] and "P " in filecont[j]:# and " - " in filecont[j]:
                    bd2.append(float(float_pattern.findall(filecont[j])[-1]))
                if "BD*(   2)" in filecont[j] and "P " in filecont[j]:# and " - " in filecont[j]:
                    bd2s.append(float(float_pattern.findall(filecont[j])[-1]))
                # elif "BD*(   2)" in filecont[j] and " P " in filecont[j]:
                #     bd2s.append(float(filecont[j].split()[8]))

                #if "----------------------" in filecont[j]:  
            try:
                bdav = round(sum(bd)/len(bd),5)
                bdsav = round(sum(bds)/len(bds),5)
                bd2av = round(sum(bd2)/len(bd2),5)
                bd2sav = round(sum(bd2s)/len(bd2s),5)
                gap1 = bdsav-float(elp)
                gapabs = min(bds+bd2s)-max(bd+bd2+[float(elp)]) 
                return(s_lp+";"+elp+";"+rys+";"+str(bdav)+";"+str(max(bd))+";" +str(len(bd))+";"+str(bdsav)+";"+str(min(bds))+";"+str(len(bds))+";"+str(gap1)+";"+str(gapabs)+";"+str(bd2av)+";"+str(max(bd2))+";"+str(len(bd2))+";"+str(bd2sav)+";"+str(min(bd2s))+";"+str(len(bd2s))+";","")
                
            except:    
                bdav = round(sum(bd)/len(bd),5)
                bdsav = round(sum(bds)/len(bds),5)
                gap1 = bdsav-float(elp)
                gapabs = min(bds)-max(bd+[float(elp)]) 
                return(s_lp+";"+elp+";"+rys+";"+str(bdav)+";"+str(max(bd))+";" +str(len(bd))+";"+str(bdsav)+";"+str(min(bds))+";"+str(len(bds))+";"+str(gap1)+";"+str(gapabs)+";"+";;;;;;","")
    return("0;0;0;0;","")
...
# to limit output to the desired vibrations, indicating the appropriate frequency and intensity range in the script is required
    ...
    for atom in specs:
        if not atom.isdigit():
            error += atom + ": Only numbers accepted as input for distances "
        if int(atom) > frq_len-7:
            error += atom + " is out of range. Maximum valid atom number: " + str(frq_len-7) + " "
    if error != "": return("",error+int(len(specs)/2)*";")
    for i in range(int(len(specs)/2)):
        for vib in range(len(data)):
            if (data[vib].deltas[int(specs[2*i])-1] > threshold and data[vib].deltas[int(specs[2*i+1])-1] > threshold) and data[vib].freq < freqmax and data[vib].freq > freqmin and data[vib].int > intmin and data[vib].int < intmax:
...
"g":        [" g:        Thermochemical data from Freq. Output: E_HF, G                           - 2",get_enthalpies,get_filecont],
...
"nbo":      [" nbo:      Natural charge based on NBO                                              - n(extra input)",get_nbo,get_filecont],
"nborbs":   [" nborbs:   Natural orbitals                                                         - 2",get_nbo_orbs,get_filecont],
"nborbsP":  [" nborbsP:   Natural orbitals of P                                                   - 4",get_nbo_orbsP,get_filecont],
"nmr":      [" nmr:      Isotropic NMR shift                                                      - n(extra input)",get_nmr,get_filecont],
"nmrtens":  [" nmrtens:  Eigenvalues of the Anisotropic NMR shift tensor                          - 3*n(extra input)",get_nmrtens,get_filecont],
"dihedral": [" dihedral: Dihedral angles                                                          - n(extra input)",get_dihedrals,get_outstreams],
"angle":    [" angle:    Angles                                                                   - n(extra input)",get_angles,get_outstreams],
"dist":     [" dist:     Distances                                                                - n(extra input)",get_distances,get_outstreams],
"sterimol": [" sterimol: Sterimol L, B1, B5                                                       - 3",run_sterimol2,return_log],
"dipole":   [" dipole:   Total dipole moment in Debye from the last relevant job in the file      - 1",get_dipole,get_filecont],
"polar":    [" polar:    'Exact' Polarizability from the last relevant job in the file (keyword: dipole)   - 2",get_polarizability,get_filecont],
"volume":   [" volume:   Molar volume in bohr^3/mol (keyword: volume)                             - 1",get_volume,get_filecont],
"surface":  [" surface:  Cavity surface area in Ang^2 and volume in Ang^3 from solvation model    - 2",get_cavity,get_filecont],
"ir":       [" ir:       ir                                                                       - n(extra input)",get_ir,get_filecont],
"overlap":  [" overlap:  Overlap energy of LP(O) with BD*(NH) in kcal/mol                         - n(occurence)",get_nbo_overlap,get_filecont],
...
"allsterimol": [" allsterimol: Sterimol L, B1, B5 for all substituents at the specified atom         - 3*n(subst)",run_allsterimol,return_log],
"fukui":    [" fukui:    Atom-condensed Fukui +/- indices                                         - 2*n",get_fukui,get_filecont],
"hirsh":    [" hirsh:    Hirshfeld charge, CM5 charge, Hirshfeld atom dipole                      - 4*n",get_hirsh,get_filecont], 
"chelpg":   [" chelpg:   ChelpG ESP charge                                                        - n",get_chelpg,get_filecont], 
"qpole":    [" qpole:    Quadrupole moment eigenvalues and amplitude                              - 4",get_quadrupole,get_outstreams],
"efg":      [" efg:      Electric field gradient tensor min and max eigenvalues                   - 2",get_efg,get_filecont],
"pyr":      [" pyr:      Pyramidalization of first atom wrt the next three                        - 1",get_pyramidalization,get_outstreams],
"route":    [" route:    Route lines                                                              - n(jobs)",get_route,get_outstreams],
"nbohomo":  [" nbohomo:  HOMO, LUMO based on NBO analysis                                         - 2",get_nbohomolumo,get_filecont],
...
#Reads a table wiht the atom lables from Gaussian and designatesand makes the numbers into the numbers selected by the get_atominput function
inputs = pd.read_excel("Atomlabel.xlsx")
inputs = inputs.drop(['Name'], axis=1)
columnnames= list(inputs.columns)
columnnames= columnnames + columnnames
inputsaslist= inputs.values.tolist()
...
def main(jobs):
    ...
    for job in jobs:
        ...
        for log in range(len(logs)):
            inputsaslist[log]= [str(i) for i in inputsaslist[log]]
            atoms = inputsaslist[log]
            if readgjf:
                filecont = get_coords_com(logs[log]) # returns coords
                #filecont,error = jobtypes[job][2](log)
                error = ""
            else:
                filecont,error = jobtypes[job][2](logs[log])
            if watchout(log,error):
                continue
            content,error = jobtypes[job][1](filecont,atoms) #5,6,7
            if error != "":
                content = error
            #print(content)
            #print_results(logs[log],content)
            results[logs[log]].append(content)
    resultslist = (pd.DataFrame.from_dict(results, orient='index')).values.tolist()
    resultssorted = []
    print(resultssorted)
    for i in range(len(resultslist)):
        entry = (resultslist[i][0]).split(';')
        entry = entry[:-1]
        entryatomlable = entry[::2] + entry[1::2]
        resultssorted.append(entryatomlable)
    resultsdataframe = pd.DataFrame(resultssorted)
    #resultsdataframe.columns = columnnames
    print(resultsdataframe)
    resultsdataframe.to_excel('data.xlsx')
